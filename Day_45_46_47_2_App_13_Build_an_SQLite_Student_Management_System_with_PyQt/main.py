...
class DataBaseConnection:
    def __init__(self, data_base_file="database.db"):
        self.data_base_file = data_base_file

    def connect(self):
        connection = sqlite3.connect(self.data_base_file)
        return connection
...
class EditDialog(QDialog):
    def __init__(self):
        ...
        self.students_id = main_window.table.item(index, 0).text()
    ...
    def update_student(self):
        ...
        cursor.execute("UPDATE students SET name = ?, course = ?, mobile = ? WHERE id = ?",
                       (self.student_name.text(), self.course_name.currentText(),
                        self.mobile.text(), self.students_id))
        connection.commit()
        cursor.close()
        connection.close()
...
class DeleteDialog(QDialog):
    ...
    def delete_student(self):
        ...
        index = main_window.table.currentRow()
        students_id = main_window.table.item(index, 0).text()

        connection = sqlite3.connect("database.db")
        cursor = connection.cursor()
        cursor.execute("DELETE FROM students WHERE id = ?", (students_id,))
        connection.commit()
        cursor.close()
        connection.close()
        main_window.load_data()
        ...
        confirmation_message.setText("Student deleted successfully")
...
class AboutDialog(QMessageBox):
    def __init__(self):
        ...
        content = """
        This app was created during the "Python mega course" from Udemy
